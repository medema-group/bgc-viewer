...
app = Flask(__name__, static_folder=str(frontend_build_dir), static_url_path="/static")
...
if PUBLIC_MODE:
    ...
    if not secret_key:
        raise RuntimeError(
            "BGCV_SECRET_KEY environment variable must be set in PUBLIC_MODE."
        )
...
def index():
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": "Frontend not built or not included in package. Run 'npm run build' in the frontend directory."
                }
            ),
            404,
        )
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify(
                    {
                        "error": "Frontend not found - ensure 'npm run build' was executed and static files are included in package"
                    }
                ),
                404,
            )
        ...
        try:
            ...
            for item in sorted(resolved_path.iterdir()):
                try:
                    if item.is_dir():
                        items.append(
                            {"name": item.name, "type": "directory", "path": str(item)}
                        )
                    elif (
                        item.suffix.lower() == ".json"
                        or item.name.endswith(".json.gz")
                        or item.name.endswith(".json.bz2")
                    ):
                        items.append(
                            {
                                "name": item.name,
            ...
            return jsonify({"current_path": str(resolved_path), "items": items})
                                ...
                                json_files.append(
                                    {
                                        ...
                                        "directory": (
                                            str(
                                                json_file.parent.relative_to(
                                                    resolved_path
                                                )
                                            )
                                            if json_file.parent != resolved_path
                                            else "."
                                        ),
                                    }
                                )
    ...
    try:
        ...
        if PUBLIC_MODE:
            ...
            if not Path(db_path).exists():
                return (
                    jsonify({"error": f"Database file does not exist: {db_path_str}"}),
                    404,
                )
            ...
            try:
                ...
                if "error" not in db_info:
                    ...
                    if not data_root:
                        return (
                            jsonify({"error": "Database metadata missing data_root"}),
                            500,
                        )
                else:
                    return (
                        jsonify(
                            {
                                "error": f"Failed to read database metadata: {db_info.get('error')}"
                            }
                        ),
                        500,
                    )
                ...
                return (
                    jsonify(
                        {"error": f"Invalid data_root in database metadata: {str(e)}"}
                    ),
                    500,
                )
            build_id = record_cache_build_id(db_path, db_info)
        ...
        if PUBLIC_MODE:
            try:
                ...
                return (
                    jsonify(
                        {
                            "error": "Access denied: File must be within the data root folder"
                        }
                    ),
                    403,
                )
        ...
        if not file_path.exists():
            return (
                jsonify({"error": f"File {filename} not found in database folder"}),
                404,
            )
        ...
        if not modified_data:
            return (
                jsonify({"error": f"Record {record_id} not found in {filename}"}),
                404,
            )
...
def get_record_regions(record_id):
    ...
    record = next(
        (r for r in antismash_data.get("records", []) if r.get("id") == record_id), None
    )
    if not record:
    ...
    for feature in record.get("features", []):
        if feature.get("type") == "region":
            ...
            region_info = {
                ...
                "region_number": feature.get("qualifiers", {}).get(
                    "region_number", ["unknown"]
                )[0],
    ...
    return jsonify(
        {"record_id": record_id, "regions": sorted(regions, key=lambda x: x["start"])}
    )
...
def get_region_features(record_id, region_id):
    ...
    for feature in record.get("features", []):
        if (
            ...
            and f"region_{feature.get('qualifiers', {}).get('region_number', [''])[0]}"
            == region_id
        ):
...
def get_version():
    ...
    return jsonify({"version": __version__, "name": "BGC Viewer"})
...
def get_tfbs_hits(record_id):
    ...
    if region not in hits_by_region:
        return jsonify(
            {"record_id": record_id, "region": region, "count": 0, "hits": []}
        )
...
def get_tta_codons(record_id):
    ...
    return jsonify(
        {"record_id": record_id, "count": len(tta_codons), "codons": tta_codons}
    )
...
def get_resistance_features(record_id):
    ...
    for locus_tag, hit_data in best_hits.items():
        resistance_features.append({"locus_tag": locus_tag, **hit_data})
...
def search_protocluster_endpoint():
    ...
    results = search_protoclusters(
        index, req.query, offset=req.offset, limit=req.per_page
    )
        ...
        try:
            ...
            if not resolved_index_path.parent.exists():
                try:
                    ...
                    return (
                        jsonify(
                            {"error": f"Failed to create index directory: {str(e)}"}
                        ),
                        400,
                    )
            ...
            if selected_files and len(selected_files) > 0:
                ...
                if not json_files_to_process:
                    return (
                        jsonify(
                            {"error": "None of the selected files are valid JSON files"}
                        ),
                        400,
                    )
...
def run_preprocessing(folder_path, index_path, json_files=None):
    ...
    try:
        ...
        results = preprocess_antismash_files(
            folder_path, index_path, progress_callback, json_files
        )
        ...
        PREPROCESSING_STATUS.update(
            {"is_running": False, "status": "error", "error_message": str(e)}
        )
...
def main():
    ...
    if session_type == "redis":
        ...
        print(
            "WARNING: Redis not available in PUBLIC_MODE, using filesystem sessions as fallback"
        )
