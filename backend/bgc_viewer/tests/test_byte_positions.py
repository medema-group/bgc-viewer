...
def test_byte_positions_coverage(test_database):
    ...
    assert (
        invalid_count == 0
    ), f"Found {invalid_count} records with invalid byte positions"
    ...
    assert (
        zero_size_count == 0
    ), f"Found {zero_size_count} records with zero-size byte ranges"
