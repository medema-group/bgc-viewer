...
class TestDatabaseSearch:
    ...
    def test_search_by_organism(self, processed_data_dir):
        ...
        results = get_database_entries(
            db_path, page=1, per_page=10, search="Streptomyces"
        )
    ...
    def test_search_by_product(self, processed_data_dir):
        ...
        results = get_database_entries(
            db_path, page=1, per_page=10, search="polyketide"
        )
    ...
    def test_search_nonexistent_term(self, processed_data_dir):
        ...
        results = get_database_entries(
            db_path, page=1, per_page=10, search="nonexistent_term_xyz"
        )
        ...
        if results_page1["total"] > 1:
            ...
            if len(results_page1["entries"]) > 0 and len(results_page2["entries"]) > 0:
                ...
                assert (
                    ...
                    or entry1["record_id"] != entry2["record_id"]
                )
...
class TestSearchAttributeValues:
    ...
    def test_search_in_attribute_values(self, processed_data_dir):
        ...
        for search_term in test_searches:
            results = get_database_entries(
                db_path, page=1, per_page=10, search=search_term
            )
    ...
    def test_case_insensitive_search(self, processed_data_dir):
        ...
        results_lower = get_database_entries(
            db_path, page=1, per_page=10, search="test"
        )
        results_upper = get_database_entries(
            db_path, page=1, per_page=10, search="TEST"
        )
        results_mixed = get_database_entries(
            db_path, page=1, per_page=10, search="Test"
        )
