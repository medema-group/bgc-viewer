...
class TestFullSystemIntegration:
    ...
    def test_preprocessing_to_loading_pipeline(self, temp_dir, sample_antismash_data):
        ...
        loaded_record1 = load_specific_record(
            str(test_file), "test_record_1", str(temp_dir)
        )
        loaded_record2 = load_specific_record(
            str(test_file), "test_record_2", str(temp_dir)
        )
    ...
    def test_multiple_files_processing(self, temp_dir, sample_antismash_data):
            ...
            (
                "file1.json",
                {
                    **sample_antismash_data,
                    "records": [
                        sample_antismash_data["records"][0]
                    ],  # First record only
                },
            ),
            (
                "file2.json",
                {
                    **sample_antismash_data,
                    "records": [
                        sample_antismash_data["records"][1]
                    ],  # Second record only
                },
            ),
        ]
        ...
        record1 = load_specific_record(
            str(created_files[0]), "test_record_1", str(temp_dir)
        )
        record2 = load_specific_record(
            str(created_files[1]), "test_record_2", str(temp_dir)
        )
    ...
    def test_preprocessing_with_progress_callback(self, temp_dir, sample_json_file):
        ...
        result = preprocess_antismash_files(
            str(temp_dir), index_path, progress_callback
        )
        ...
        assert (
            progress_updates[-1]["files_processed"]
            == progress_updates[-1]["total_files"]
        )
...
class TestPerformanceAndScalability:
    ...
    def test_large_record_handling(self, temp_dir):
        ...
        test_data = {"version": "1.0", "records": [large_record]}
        ...
        assert (
            len(loaded["records"][0]["features"]) == 102
        )  # 1 region + 1 protocluster + 100 CDS

    def test_random_access_vs_fallback_performance(
        self, processed_data_dir, sample_json_file
    ):
        ...
        random_access_result = load_specific_record(
            str(sample_json_file), "test_record_1", str(temp_dir)
        )
